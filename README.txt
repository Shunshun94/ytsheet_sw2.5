...
**追加／変更**
- [共通] キャラクター一覧にタグと最終参加セッションを表示 
- [共通] 検索項目に、マイリスト内検索のチェックボックス
- [共通] シート一覧のOGP
- [共通] 作成レギュレーションの備考欄
- [共通] 過去ログからの復元編集時、メモ欄・履歴欄に最新のデータを適用するボタン
- [共通] Tekey向けのチャットパレットを出力するオプション
- [共通] 閲覧側でカラーカスタムをOFFにするオプション
- [共通] スクロールの状態が一番上でない時、ヘッダをクリックするとページの一番上に戻す仕様
- [共通] ルビの含まれたテキストをコピーした際、ルビ部分を()で囲われるようにする仕様
- [共通] 各シートにおける、ヘッダ部分を固定
- [共通] 各種アイコンをFontAwesomeからGoogleMaterialSymbolsのものへ
- [共通] 一覧の性別アイコンをGoogleMaterialSymbolsのものへ
...
**調整／改善**
- [共通] details（テキスト折り畳み全般）の外観や挙動の微調整
- [共通] キャラクター画像のURL
- [共通] チャットパレットにおいて、ゆとチャ向けはダイスコードを行末尾に出力する
- [共通] モバイルブラウザにおける、PC版サイト閲覧モードのフォントサイズ自動調節を無効化
- [共通] モバイル閲覧時（特に編集画面）における、各項目の間隔（特に上下のmargin）を広げる
...
**不具合修正**
以下の不具合修正
- [共通] テキスト装飾の説明で、「透明」の項目が重複している（２つある）
- [共通] チャットパレットで、「全ての変数を出力する」オプションが（チェックの有無に関わらず）常にONだった
- [共通] キャラクター画像がない時にも（存在しない）画像へのアクセスが発生していた
- [共通] キャラクター名において、`:`があったり特定のテキスト装飾を用いるとレイアウトが崩れる
- [共通] JSONファイルからインポートしたとき、最初の編集画面でのみ画像が表示されてしまう
- [共通] テキスト装飾において、あいだに改行があってもルビ記法として解釈されてしまう
...
**内部的な影響のみの修正**
- [共通] 閲覧時の画像表示の処理を共通化
- [共通] セッション履歴周りのCSSを共通化
- [共通] タグ置換（テキスト装飾）の処理を共通化
...
**不具合修正／調整／改善**
- [共通] 過去ログの命名ボタンの表記を変更
- [共通] シート保存時、編集保護設定を変更した際に設定とディレクトリがズレることがある不具合を修正
- [共通] コメントアウト記法（ `//...` ）を使用したときに改行が残らないように修正
