...
### 技能データ各種変換 --------------------------------------------------
my @spellNames;
push(@spellNames, $data::class{$_}{magic}) foreach(grep { $data::class{$_}{magic} } @data::class_names);
...
print <<"HTML";
  <section id="section-common">
      ...
      <div class="box" id="classes">
        ...
        <div class="classes-group" id="classes-weapon-user">
          ...
          <dl class="edit-table side-margin">
            @{[ map { classInputBox($_) } grep { $data::class{$_}{type} =~ 'warrior' } @data::class_names ]}
          </dl>
        </div>
        <div class="classes-group" id="classes-magic-user">
          <h3>呪文使い系</h3>
          <dl class="edit-table side-margin">
            @{[ map { classInputBox($_) } grep { $data::class{$_}{type} =~ 'spell' } @data::class_names ]}
          </dl>
        </div>
      </div>
    </div>
    ...
    <details class="box" id="general-skills" $open{generalSkill}>
      ...
      @{[ renderAddDelButtons('general-skill') ]}
      <ul class="annotate">
        <li>「自動」にチェックを入れると、習得段階「初歩」で消費する成長点を0として計算します。<br>
          （経歴や種族によって自動的に習得する技能で、チェックを入れてください）
      </ul>
    ...
    <div class="box" id="spell-cast">
      ...
      <table class="edit-table">
        ...
        @{[ map {
          ...
          ROW
        } grep { $data::class{$_}{type} =~ 'spell' } @data::class_names ]}
      </table>
    </div>
          ...
          @{[ renderTemplateLoop(
            ...
            sub ($num) {
              ...
              <tr id="spell-row${num}">
                ...
                <td class="system"><select name="spell${num}System">@{[ option "spell${num}System", @spellNames ]}</select>
        ...
        <table class="edit-table line-tbody">
          ...
          <tbody>
            ...
            @{[ map {
              ...
              ROW
            } grep { $data::class{$_}{type} =~ 'warrior' } @data::class_names ]}
        </table>
      </div>
            ...
            sub ($num) {
              ...
              <tbody id="weapon-row$num">
                <tr>
                  ...
                  <td rowspan="2"><span class="button" onclick="addWeapon(${num});">複<br>製</span>
                <tr>
...
sub classInputBox {
  ...
  return qq|<dt id="class${id}">|
  . qq|<ruby>$name<rt>$data::class{$name}{kana}</ruby>|
  . '<dd>' . (input "lv${id}", 'number','changeLv','min="0" max="10"');
}
sub renderDataList {
  return <<~"HTML";
